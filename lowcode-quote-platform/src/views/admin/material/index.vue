<template>
  <div class="admin-container">
    <el-container class="work-bench">
      <el-header height="60px" class="workbench-header">
        <div class="logo">
          <el-icon :size="20"><Setting /></el-icon>
          <span>印刷包装平台 - 📦全部业务物料管理中心</span>
        </div>
        <div class="header-actions">
           <el-button type="primary" plain @click="$router.push('/admin/category')">⬅️ 返回品类与图纸台</el-button>
           <el-button color="#2b333e" text @click="$router.push('/')">退出返回商城首页</el-button>
        </div>
      </el-header>

      <el-container class="workbench-body" style="padding: 20px;">
        <el-card style="width: 100%;">
          <div style="margin-bottom: 5px; display: flex; justify-content: space-between; align-items:center;">
             <h3 style="margin: 0; padding: 0;">配置全局业务属性组合（此处增减将在图纸台左侧列表同步）</h3>
          ...
          <el-table :data="activeComponentList" style="width: 100%" height="calc(100vh - 250px)" border>
...
<style scoped>
/* 偷懒复用 admin-container 样式，保持一致美观 */
.admin-container {
  height: 100vh;
  display: flex;
  flex-direction: column;
}
.work-bench { height: 100%; border-radius: 6px; overflow: hidden; }
.workbench-header {
